...
- **"Could not be checked in time" is not a failure.** Assets whose fetch timed out come back as *unchecked* and never fail the probe — a slow link says nothing about the file (the first version of this check called two healthy Next.js chunks broken from a distant client). Re-run the probe, or open that URL yourself, before treating one as a problem.
