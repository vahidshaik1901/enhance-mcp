...
### Task 9: Path-clash guard, root apps, and the mixed-site guidance (added 2026-09-17 after the walkthrough)

**Why:** the walkthrough and a user question exposed three gaps. (1) A proxy path silently shadows a same-named `public_html` directory (verified: an app on `demo-login` turned a live PHP page into a 503), and today only prose warns about it. (2) The panel accepts an **empty proxy path**, which hands the WHOLE site to the app (verified 2026-09-17 on vahid2.dev: `/`, `/anything/deep` and `/index.html` all answered 503 while a stopped app owned `""`; `"/"` and `"."` are a 400; deleting the app restored the docroot). That is the right layout for a Node app that is the whole site, on its own website or subdomain, and the tools cannot express it (`proxy_path` has `min(1)` and the path regex rejects `""`). (3) Under a path the proxy strips the prefix, so anything an app references by absolute URL (files in `public/`, `<Link href="/about">`, `fetch('/api')`) goes to the domain root: the user saw broken images on `/next/` because `assetPrefix` only covers `/_next/static` bundles.

**Files:**
- Modify: `server/src/tools/apps.ts`, `server/test/unit/tools-apps.test.ts`
- Modify: `server/test/e2e/milestone-c.e2e.test.ts` (one assertion)
- Modify: `skills/enhance-deploy/SKILL.md`, `docs/research.md`, `README.md`, `CLAUDE.md`

**Interfaces:**
- Consumes: `ctx.httpProbe ?? httpsProbe` (`core/probe.ts`), `appsSite`, `listApps`, `validateProxyPath`, `appUrl`.
- Produces: `pathPreflight(ctx, w, path): Promise<{ status: number | null; taken: boolean; detail: string }>`; new create arguments `serve_at_root: boolean` (default false) and `replace_existing_path: boolean` (default false); the same `replace_existing_path` on update.

**Behaviour:**
- `serve_at_root: true` registers the app with the panel's empty proxy path (`proxyDetails.path = ""`), so the app answers on the whole domain and receives the full request path (nothing to strip). It is mutually exclusive with `proxy_path` (refuse when both are given) and still requires `port`. `appUrl` returns `https://<domain>/` for it. `validateProxyPath` keeps rejecting `""`; the empty path is reachable only through `serve_at_root`. The listing renders a root app's proxy as `/ (whole site) → :<port>`.
- **Preflight.** Before the POST in `persistent_app_create` whenever the app gets a proxy (path or root), and before the PATCH in `persistent_app_update` whenever the proxy path changes to a new value: fetch `https://<serverIp>/<path>/` (root: `/`) with SNI and Host set to the primary domain through the existing probe transport (5 s, 512 bytes). `status === 404` or a connection failure means the path is free: proceed. Any other status means something already answers there (a PHP page, a static file, a directory listing, a redirect, or another app): return `fail(...)` with the identity block, the status seen, and `Registering this app would replace what https://<domain>/<path>/ serves today (HTTP <status>). Nothing was sent to the panel. Pick a path that returns 404 now, or pass replace_existing_path=true if replacing it is intended.` With `replace_existing_path: true` the preflight still runs, the write proceeds, and the success text records what was replaced. A probe error never blocks the write; it adds a note that the path could not be checked.
- For `serve_at_root` the refusal text is stronger: `This website already serves content at its root (HTTP <status>). A root app takes over the ENTIRE site, including every PHP and static page. Use a dedicated website or subdomain for a whole-site Node app, or pass replace_existing_path=true.`
- The success text of a root app says the app owns the whole domain and that PHP/static files in `public_html` are no longer served while it is registered.

- **Asset check in `persistent_app_probe`** (user requirement, 2026-09-17: a customer who deploys and sees broken images will panic, so the plugin must catch it, not the customer). A page can answer 200 while its images are broken (seen live: `/next/` was 200 while `/next.svg` was 404). When the probed response is `text/html` and the status is 2xx, the probe re-fetches the page with a 64 KB body cap, extracts up to 12 distinct same-origin URLs from `<img src>`, `<script src>`, `<link rel="stylesheet|icon|preload" href>` and the first candidate of each `srcset` (absolute-path and relative URLs only; skip `data:`, `#`, other hosts), resolves them against the page URL, and fetches each through the same transport (2 s deadline, 1 byte cap, in parallel). It returns `assets: { checked, failed: [{ url, status, outsidePrefix }] }` and, when any failed, turns the result into `fail(...)` with `reachable: true` kept in structured content and a plain-language list: for a failed URL outside `/<path>/` say `requested at the domain root, outside /<path>/, where this site's own files are served: reference it as /<path><url> or move the app to its own website/subdomain (serve_at_root)`; for one inside the prefix say the app itself does not serve it. `check_assets: boolean` argument, default true. A pure exported `extractAssetUrls(html: string, pageUrl: string): string[]` carries the parsing and gets its own unit tests (absolute path, relative path, srcset, data: skipped, other host skipped, dedupe, cap at 12).
- Unit tests for the probe asset check (fake `ctx.httpProbe` that answers by path): all assets 200 → ok with `assets.checked === n`; one root-level asset 404 → `isError`, text names the URL and the `/<path>` rewrite, `failed[0].outsidePrefix === true`; non-HTML response → no asset requests; `check_assets: false` → no asset requests.

- **Fix the 404 hint (Task 8b review, Important).** `persistent_app_probe` currently says "HTTP 404 came from the app itself", but the docroot answers the same 404 when NO app owns the path (seen live after `clear_proxy` and after the deletes). Make the hint conditional: with `app_id` (an app with a proxy is registered) say `HTTP 404 is most likely the app's own (the web server's failures are 502/503/504): the proxy strips the /<path> prefix, so the app received "/" — check it serves "/" (Next.js: assetPrefix, not basePath)`; with `proxy_path` only, look the path up in `persistent_apps_list` and, when no app owns it, say `no persistent app is registered on /<path>/, so this 404 is the site's own docroot answering`. Remove the absolute from the code comment. Tests: both branches, and an assertion that the real proxy path appears in the hint (`/strips the \/node prefix/`).

- [ ] **Step 1: Failing unit tests** in `tools-apps.test.ts` (inject `ctx.httpProbe`): (a) create with `proxy_path` where the fake probe answers 200 → `isError`, text contains `HTTP 200`, `replace_existing_path`, the identity line, and no POST was sent; (b) same with 404 → POST sent, no warning; (c) same with 200 and `replace_existing_path: true` → POST sent, success text says what was replaced; (d) probe throws → POST sent, text notes the path could not be checked; (e) `serve_at_root: true` with probe 404 → POST body `proxyDetails.path === ''`, `structured.url === 'https://vahi.dev/'`, the probe was asked for path `/`; (f) `serve_at_root` with probe 200 → refusal containing `ENTIRE site`; (g) `serve_at_root` together with `proxy_path` → refusal, nothing probed or sent; (h) update changing `proxy_path` to a taken path → refusal, no PATCH; update not touching the path → no preflight at all (the fake probe throws `must not be called`); (i) the listing renders a root app as `/ (whole site)` with `url: 'https://vahi.dev/'`.
- [ ] **Step 2:** run them, see them fail for the right reason.
- [ ] **Step 3: Implement** in `apps.ts`. Keep `pathPreflight` a small exported function; reuse the probe request shape `{ ip, host, path, timeoutMs: 5000, maxBodyBytes: 512 }`; no server IP recorded → treat as "could not be checked". Update the create/update descriptions with one sentence each about the preflight and `replace_existing_path`, and create's with `serve_at_root`.
- [ ] **Step 4:** focused tests green and pristine; then once `npm run typecheck && npm test && npm run build`.
- [ ] **Step 5: e2e** (`milestone-c.e2e.test.ts`): before creating its app, assert that a create on the proxy path `demo-login` is refused by the preflight on vahi.dev only when `process.env.ENHANCE_E2E_TAKEN_PATH` is set (use that variable's value as the path; skip the assertion when unset, so the suite stays site-agnostic). Do not run the live suite in this task unless the controller says the credential is fresh; skip-mode and typecheck are required.
- [ ] **Step 6: Skill.** First, in the Node path's verify step: run `persistent_app_probe` after every Node deploy and treat failed assets as a failed deploy (fix the references or move the app to its own subdomain, then redeploy) before telling the customer anything is live. Also ask up front whether the project is an API/single-page app or a full multi-page site, and recommend the dedicated website/subdomain layout for the latter before deploying. Then: In `skills/enhance-deploy/SKILL.md` add a section `## PHP or static site plus a Node app` after "Node runtime": who owns which paths; choose a proxy path that returns 404 today (the create tool now refuses one that does not, and `ls public_html/<first segment>` over SSH is the exact check, since an empty directory also 404s); the reverse check before rsyncing PHP/static files into `public_html/<dir>` (`persistent_apps_list`: stop if an app owns that path); after registering, re-request the site's known URLs; most registrations restart the container, so do it at a quiet moment. Then the layout choice: **an API or a single-page app under a path is fine; a Node app that is the whole site, or any multi-page framework site, goes on its own website or subdomain with `serve_at_root=true`** — no prefix stripping, no `assetPrefix`, absolute URLs just work. Under a path, state the verified limit: `assetPrefix` covers only the `/_next/static` bundles; files in `public/`, generated links and absolute fetches still go to the domain root, so they must be written with the prefix (user-reported broken images on `/next/`, fixed by `src="/next/next.svg"`). Update Node layout step 4 to point at this section.
- [ ] **Step 7: Docs.** `docs/research.md`: Task 1 probe items 12 (empty proxy path accepted, owns the whole site, `"/"` and `"."` 400, evidence from vahid2.dev) and 13 (`assetPrefix` does not cover `public/` files; evidence: `/next.svg` 404 vs `/next/next.svg` 200); add both to the "Live test C" findings. README: mention the path-clash guard and whole-site apps in the Node bullet. CLAUDE.md live-facts list: two bullets for items 12 and 13; tool count unchanged (82).
- [ ] **Step 8:** `claude plugin validate .`; commit code+tests, then skill+docs, trailer `Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>`.
