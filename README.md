...
- **Creates that report what really happened.** A create the panel took but did not answer is confirmed by reading it back, never reported as failed. When even the read-back cannot settle it, the tool says the outcome is unknown, tells the model not to retry, and names the read that settles it, because retrying a create that landed makes a duplicate.
...
Milestone D1 (write-then-verify on every create, the read-only `files_list`, and the on-disk line in the path-clash refusal) is in progress on branch `feat/milestone-d1`.
