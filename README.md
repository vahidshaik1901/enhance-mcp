...
- **Fresh hosting to live site.** Domain check, site creation, DNS instructions built from your panel's own nameservers and zone, Let's Encrypt, SSH key setup, rsync deploy, verification on the preview URL. `files_list` shows what is in a site's folders before and after a deploy, read-only through the panel's file service, with `ls` over SSH as the fallback.
- **Creates that report what really happened.** A create the panel took but did not answer is confirmed by reading it back, never reported as failed. When even the read-back cannot settle it, the tool says the outcome is unknown and names the read that settles it; such a create is never retried until that read shows the object absent, and then only after telling you, because retrying a create that landed makes a duplicate.
...
| `ENHANCE_READ_ONLY=1` | register read tools only. `files_list` is one of them and stays available; it is the one read tool that sends a POST, to mint the short-lived site token the panel's file service needs (the tool itself only lists) |
...
Milestone D1 (write-then-verify on every create, the read-only `files_list`, and the on-disk line in the path-clash refusal) is verified live on 2026-09-24: its end-to-end suite, the milestone B and C suites as regressions, and a forced check in which a create the client gave up on was confirmed by reading it back (see "Live test D1" in `docs/research.md`).

Next: email, backups, DNS zone editing, WordPress, staging and the other deploy modes (milestones D2 to D6). OAuth support lands as soon as the panel offers it.
