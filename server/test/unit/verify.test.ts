...
describe('writeThenVerify', () => {
  ...
  it('gives up as unknown after ceil(window / interval) reads, the first one immediate', async () => {
    ...
    expect(o).toEqual({ state: 'unknown', writeError: 'fetch failed', reads: 5, elapsedMs: expect.any(Number) });
    expect(finds).toBe(5);
  ...
  it('keeps re-reading through a failing read and reports the last failure', async () => {
    ...
    expect(finds).toBe(2);
    expect(o).toEqual({ state: 'unknown', writeError: 'fetch failed', verifyError: 'HTTP 500 http_500: listing down', reads: 2, elapsedMs: expect.any(Number) });
  });
  ...
  it('forgets a read failure once a later read answers cleanly', async () => {
    ...
    expect(o).toEqual({ state: 'unknown', writeError: 'fetch failed', reads: 2, elapsedMs: expect.any(Number) });
  });

  it('stops re-reading once the window has passed on the real clock, however few reads that took', async () => {
    // Each read takes the client's full 30 s timeout: counted alone, a 90 s window at one read every
    // 5 s would be 18 reads, about ten minutes. The clock caps it at the window plus one read.
    let t = 0;
    let finds = 0;
    const pauses: number[] = [];
    const o = await writeThenVerify({
      write: async () => { throw new DOMException('The operation was aborted due to timeout', 'TimeoutError'); },
      find: async () => { finds += 1; t += 30_000; return undefined; },
      windowMs: 90_000,
      intervalMs: 5_000,
      sleep: async (ms) => { pauses.push(ms); t += ms; },
      now: () => t,
    });
    expect(finds).toBeLessThanOrEqual(4);
    expect(o).toEqual({ state: 'unknown', writeError: 'no answer before the client stopped waiting', reads: 3, elapsedMs: 100_000 });
    // No pause is taken for a read that the window no longer allows.
    expect(pauses).toEqual([5_000, 5_000]);
    const r = unknownOutcome('org: T (o)', o as Extract<typeof o, { state: 'unknown' }>, { action: 'x', settle: 'y' });
    expect(r.text).toContain('3 re-reads over 100 s did not find it');
  });

  it('never lets a frozen clock spin: the read count stays the upper bound', async () => {
    let finds = 0;
    const o = await writeThenVerify({ write: async () => { throw new TypeError('fetch failed'); }, find: async () => { finds += 1; return undefined; }, windowMs: 10_000, intervalMs: 2_000, sleep: noSleep, now: () => 0 });
    expect(finds).toBe(5);
    expect(o).toEqual({ state: 'unknown', writeError: 'fetch failed', reads: 5, elapsedMs: 0 });
  });

  it('says how many reads a read-confirmed write took, and over how long', async () => {
    let t = 1_000;
    let finds = 0;
    // Two reads of 500 ms each with a 1 s pause between them: 2 s from the first read to the find.
    const o = await writeThenVerify({ write: async () => { throw new TypeError('fetch failed'); }, find: async () => { t += 500; return ++finds === 2 ? 'found-it' : undefined; }, intervalMs: 1_000, sleep: async (ms) => { t += ms; }, now: () => t });
    expect(o).toEqual({ state: 'landed', confirmedBy: 'verify', found: 'found-it', writeError: 'fetch failed', reads: 2, elapsedMs: 2_000 });
  });
...
describe('unknownOutcome / confirmedByReadNote', () => {
  it('leads with the identity block, says UNKNOWN, forbids a retry and names the settling read', () => {
    const r = unknownOutcome('org: Test (o1)', { writeError: 'no answer before the client stopped waiting', reads: 18, elapsedMs: 92_400 }, { action: 'the create of website shop.example', settle: 'domain_check domain=shop.example' }, { created: null });
    expect(r.isError).toBe(true);
    expect(r.text.split('\n')[0]).toBe('org: Test (o1)');
    expect(r.text).toContain('OUTCOME UNKNOWN');
    // The reads that really happened and the time they really took, not the window they were given.
    expect(r.text).toContain('18 re-reads over 92 s did not find it');
    ...
    expect(r.structured).toEqual({ outcome: 'unknown', writeError: 'no answer before the client stopped waiting', reads: 18, elapsedMs: 92_400, created: null });
  });

  it('names the last read failure and appends the extra line when given', () => {
    const r = unknownOutcome('org: T (o)', { writeError: 'HTTP 502 http_502', verifyError: 'HTTP 500 http_500', reads: 1, elapsedMs: 300 }, { action: 'x', settle: 'y', extra: 'One more thing.' });
    expect(r.text).toContain('1 re-read within 1 s did not find it (the last re-read failed: HTTP 500 http_500)');
